...
impl<'a> Repl<'a> {
    ...
    pub fn run(&'a mut self) {
        ...
        let prompt_out = self.get_prompt("(>");
        loop {
            let ast = self.input();
            if *(&self.exit) {
                break;
            };
            match &mut self.interpret(ast) {
    ...
    fn read_lines(&mut self) -> String {
        ...
        'read: loop {
            ...
            lines.push('\n');
            if input.ends_with(is_left_punct) {
                continue 'read;
            } else if input.starts_with(|c| matches!(c, ':' | '.'))
    ...
    pub fn print_env(&mut self, input: Option<&str>) -> Expr {
        if let Some(cmd) = input {
            ...
            match cmd {
                ...
                "-" => self.debug = false,
                _ => println!(
                    "The print_env:{} command is not implemented",
                    cmd
                ),
            };
            self.input()
    ...
    pub fn interpret(&mut self, ast: Expr) -> Maybe<RygVal> {
        let mut result = Ok(NIL);
        // result = walk(ast.to_owned(), &mut self.scope);
        // result
        self.scope
            .walk(|mut ctx| result = walk(ast.to_owned(), &mut ctx));
        result
    }
